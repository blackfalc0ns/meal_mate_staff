...
## Strict Rebuild Control

- Never allow a widget to rebuild merely because its parent rebuilt when that widget does not depend on the changed value.

- Before using `setState()`, identify exactly which UI section depends on the changed state.

- Keep mutable state inside the smallest widget that actually needs it.

- Never call `setState()` at screen level when only one component needs updating.

- Never call `setState()` when the new value is equal to the current value.

- Never use a broad `BlocBuilder`, `BlocConsumer`, `Provider.watch`, or an equivalent listener when the widget depends on only part of the state.

- Use the project's existing selective rebuild mechanism, such as `BlocSelector`, `buildWhen`, `context.select`, `Selector`, `ValueListenableBuilder`, or its existing equivalent.

- Select only the exact primitive value or immutable object required by the widget.

- Do not watch or select an entire state object when only one field is required.

- Separate static UI from state-dependent UI so unchanged sections do not rebuild unnecessarily.

- Extract a state-dependent section into its own widget only when this creates a real and useful rebuild boundary.

- Do not recreate unchanged lists, maps, models, callbacks, decorations, or other objects during every `build`.

- Do not perform API calls, state mutations, event dispatching, navigation, dialogs, or other side effects inside `build`.

- Do not create `Future` or `Stream` instances inside `build`. Create and retain them according to the widget lifecycle.

- Avoid nested reactive builders that listen to the same state unless each builder has a distinct and justified rebuild responsibility.

- List items must rebuild independently when only one item changes whenever the existing architecture supports it.

- Use stable keys only when widget identity must be preserved. Never add keys blindly as a performance workaround.

- Do not apply speculative optimizations. Every optimization must preserve correctness, readability, and the existing architecture.

## Rebuild Validation

Before reporting completion:

- Identify every state-dependent widget affected by the implementation.

- Confirm that each listener observes only the state it needs.

- Confirm that changing one local value does not rebuild the entire screen unnecessarily.

- Confirm that static widgets remain outside unnecessary reactive builders.

- Confirm that controllers, futures, streams, callbacks, and state objects are not recreated inside `build`.

- Use Flutter DevTools rebuild tracking or the project's existing performance tooling when available.

- If rebuild behavior cannot be verified, state that clearly and do not claim that the implementation is performance-optimized.
