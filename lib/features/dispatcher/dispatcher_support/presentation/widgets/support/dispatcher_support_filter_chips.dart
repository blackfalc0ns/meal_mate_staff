...
class DispatcherSupportFilterChips extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final hasAreaChips = areaChips.isNotEmpty;
      ...
      child: ListView(
        ...
        children: [
          if (hasAreaChips)
            ...areaChips.map((chip) {
              final isAll =
                  chip.areaKey.isEmpty || chip.areaKey.toLowerCase() == 'all';
              final isSelected = isAll
                  ? (selectedArea.isEmpty ||
                        selectedArea.toLowerCase() == 'all')
                  : chip.areaKey == selectedArea;
              final label = chip.count > 0
                  ? '${chip.displayName} (${chip.count})'
                  : chip.displayName;

              return Padding(
                padding: const EdgeInsetsDirectional.only(end: Spacing.xs),
                child: InkWell(
                  onTap: () => onAreaSelected(isAll ? '' : chip.areaKey),
            ...
            ...areas.map((area) {
              ...
              final label = area == areas.first
                  ? locale.supportFilterAll
                  : area;
              return Padding(
                padding: const EdgeInsetsDirectional.only(end: Spacing.xs),
                child: InkWell(
