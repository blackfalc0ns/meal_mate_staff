...
void main() {
  group('resolveDriverAuthDestination Table-Driven Tests', () {
    test('when driver account does not exist -> returns DriverRegistrationDestination', () {
      ...
      expect((destination as DriverRegistrationDestination).phone, '+96550111222');
    });

    test('when isFirstTimeSetup is true -> returns DriverFirstTimeOtpDestination', () {
      const lookup = PhoneLookupResultEntity(
        exists: true,
    ...
    test('when applicationStatus is stage 1 (Submitted) -> returns underReview destination', () {
      const lookup = PhoneLookupResultEntity(
        exists: true,
        isFirstTimeSetup: false,
        role: UserRole.driver,
    ...
    test('when applicationStatus is stage 2 (NeedsChanges) -> returns moreInformationRequired destination', () {
      const lookup = PhoneLookupResultEntity(
        exists: true,
        isFirstTimeSetup: false,
        role: UserRole.driver,
    ...
    test('when applicationStatus is stage 3 (Rejected) -> returns rejected destination', () {
      const lookup = PhoneLookupResultEntity(
        exists: true,
        isFirstTimeSetup: false,
        role: UserRole.driver,
    ...
    test('when driver is Active and isFirstTimeSetup is false -> returns DriverPasswordLoginDestination', () {
      const lookup = PhoneLookupResultEntity(
        exists: true,
        isFirstTimeSetup: false,
        role: UserRole.driver,
