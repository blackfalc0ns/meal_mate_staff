...
import 'package:meal_mate_delivery/features/dispatcher/dispatcher_driver_performance/presentation/manager/driver_performance_state.dart';
...
class MockDriverPerformanceRepository implements DriverPerformanceRepository {
  ...
  DriverPerformanceOverviewEntity dummyOverview = const DriverPerformanceOverviewEntity(
  ...
  DriverPerformanceComparisonEntity dummyComparison = const DriverPerformanceComparisonEntity(
    period: DriverPerformancePeriod.last7Days,
    periodText: 'Last 7 days',
    dateRangeText: 'May 1 - May 7',
    drivers: [
...
void main() {
  group('DriverPerformanceViewModel Tests', () {
    ...
    test('lazy loads comparison on first tab activation and caches on second activation', () async {
      viewModel.doIntent(const LoadDriverPerformanceOverviewEvent());
      await pumpEventQueue();
      ...
      viewModel.doIntent(const SelectDriverPerformanceTabEvent(DriverPerformanceTabType.compareDrivers));
      ...
      viewModel.doIntent(const SelectDriverPerformanceTabEvent(DriverPerformanceTabType.overview));
      await pumpEventQueue();
    ...
    test('period change clears old-period data and reloads active tab', () async {
      viewModel.doIntent(const LoadDriverPerformanceOverviewEvent());
      await pumpEventQueue();
      expect(viewModel.state.overview, isNotNull);

      // Change period to Last30Days
      viewModel.doIntent(const SelectDriverPerformancePeriodEvent(DriverPerformancePeriod.last30Days));
      expect(viewModel.state.overview, isNull);
      expect(viewModel.state.comparison, isNull);
      expect(viewModel.state.selectedPeriod, DriverPerformancePeriod.last30Days);
      expect(viewModel.state.isOverviewInitialLoading, isTrue);
    ...
    test('refresh failure preserves existing content and populates refreshFailure', () async {
      viewModel.doIntent(const LoadDriverPerformanceOverviewEvent());
      await pumpEventQueue();
      expect(viewModel.state.overview, isNotNull);
    ...
    test('stale request generation ignored after rapid period changes', () async {
      repository.delay = const Duration(milliseconds: 50);

      viewModel.doIntent(const SelectDriverPerformancePeriodEvent(DriverPerformancePeriod.today));
      // Immediately switch to ThisMonth before today finishes
      viewModel.doIntent(const SelectDriverPerformancePeriodEvent(DriverPerformancePeriod.thisMonth));

      await Future.delayed(const Duration(milliseconds: 120));

      expect(viewModel.state.selectedPeriod, DriverPerformancePeriod.thisMonth);
      expect(viewModel.state.overview, isNotNull);
    });
    ...
    test('local sort toggles direction and sorts driversTable', () async {
      ...
      viewModel.doIntent(const SortDriverPerformanceTableEvent(DriverPerformanceSortField.delivered));
      // First click sorts descending
      expect(viewModel.state.sortField, DriverPerformanceSortField.delivered);
      expect(viewModel.state.sortedDriversTable.first.driverId, 'dr_2'); // 25 delivered

      // Second click on same field toggles to ascending
      viewModel.doIntent(const SortDriverPerformanceTableEvent(DriverPerformanceSortField.delivered));
      expect(viewModel.state.sortAscending, isTrue);
      expect(viewModel.state.sortedDriversTable.first.driverId, 'dr_1'); // 15 delivered
    });
  });
}
