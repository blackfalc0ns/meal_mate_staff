...
void main() {
  group('DriverDetailsShimmer & Section Shimmers', () {
    testWidgets('DriverDetailsShimmer mirrors all sections with shape-matched shimmers', (
      tester,
    ) async {
  ...
  group('Empty & Offline States', () {
    testWidgets('DriverDetailsBoxesCard renders localized empty copy when boxes list is empty', (
      tester,
    ) async {
      await tester.pumpWidget(
        createTestApp(
    ...
    testWidgets('DriverDetailsBoxesCard renders Arabic empty copy', (tester) async {
      await tester.pumpWidget(
        createTestApp(
          const DriverDetailsBoxesCard(boxes: []),
    ...
    testWidgets('DriverDetailsLocationCard renders offline state with Retry when coords are null', (
      tester,
    ) async {
      ...
      expect(find.text('Location unavailable (Driver Offline)'), findsAtLeastNWidgets(1));
  ...
  group('Avatar Initials & Name Fallbacks', () {
    testWidgets('DriverDetailsProfileCard shows initials when avatarUrl is null', (
      tester,
    ) async {
      ...
      await tester.pumpWidget(createTestApp(const DriverDetailsProfileCard(profile: profile)));
    ...
    testWidgets('DriverDetailsProfileCard shows person icon fallback when fullName is empty', (
      tester,
    ) async {
      const profile = DriverProfileEntity(
        driverId: 'd1',
  ...
  group('Disabled Actions & Fallbacks', () {
    testWidgets('DriverDetailsActionButtons disables buttons when callbacks are null', (
      tester,
    ) async {
      await tester.pumpWidget(
        createTestApp(
    ...
    testWidgets('DriverDetailsKpiRow renders zero metrics correctly', (tester) async {
      ...
      await tester.pumpWidget(createTestApp(const DriverDetailsKpiRow(kpis: kpis)));

      expect(find.text('0'), findsAtLeastNWidgets(2));
      expect(find.text('0.0'), findsOneWidget);
    });

    testWidgets('DriverDetailsPerformanceCard renders zero performance summary', (tester) async {
      ...
      await tester.pumpWidget(
        createTestApp(const DriverDetailsPerformanceCard(dailySummary: dailySummary)),
      );
  ...
  group('Responsive 320x640 Screen Viewport', () {
    testWidgets('DriverDetailsShimmer does not overflow on 320x640 viewport', (tester) async {
