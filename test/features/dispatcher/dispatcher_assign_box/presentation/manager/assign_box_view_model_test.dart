...
void main() {
  ...
  group('AssignBoxViewModel - Details and Selection', () {
    test(
      'initial load sets isInitialLoading and selects bestSuggestion by default',
      () async {
        boxRepo.detailsCompleter =
            Completer<ApiResult<AssignBoxDetailsEntity>>();

        final loadFuture = viewModel.doIntent(const LoadAssignBoxEvent());
        expect(viewModel.state.isInitialLoading, isTrue);

        boxRepo.detailsCompleter!.complete(
          const ApiSuccessResult(data: sampleDetails),
        );
        await loadFuture;
    ...
    test(
      'initial load falls back to first candidate if bestSuggestion is null',
      () async {
    ...
    test(
      'initial load with no candidates leaves selectedDriverId null and canSubmit false',
      () async {
    ...
    test(
      'SelectAssignBoxDriverEvent updates single selection locally without API call',
      () async {
    ...
    test(
      'refresh preserves selected driver if still in candidates, else falls back',
      () async {
        boxRepo.nextDetailsResult = const ApiSuccessResult(data: sampleDetails);
        await viewModel.doIntent(const LoadAssignBoxEvent());
        await viewModel.doIntent(const SelectAssignBoxDriverEvent(driverId2));
        ...
        boxRepo.nextDetailsResult = const ApiSuccessResult(
          data: detailsWithoutDriver2,
        );
        await viewModel.doIntent(const RefreshAssignBoxDetailsEvent());
        expect(
          viewModel.state.selectedDriverId,
          driverId1,
        ); // fell back to bestSuggestion
      },
    );
  });
  ...
  group('AssignBoxViewModel - Lazy Summary Caching and Retry', () {
    test(
      'loads summary on first tap, caches it, and ignores duplicate calls',
      () async {
    ...
    test(
      'ignores duplicate load summary tap while first call is in flight',
      () async {
        boxRepo.summaryCompleter =
            Completer<ApiResult<AssignBoxSummaryEntity>>();
  ...
  group('AssignBoxViewModel - Submit Assignment', () {
    test(
      'submits with localizedNotes and selected driver, increments successId on success',
      () async {
        boxRepo.nextDetailsResult = const ApiSuccessResult(data: sampleDetails);
        await viewModel.doIntent(const LoadAssignBoxEvent());
        ...
        await viewModel.doIntent(
          const SubmitAssignBoxEvent(
            localizedNotes: 'إسناد سريع للبوكس #BX-1256',
          ),
        );
        ...
        expect(
          viewModel.state.assignmentResult?.message,
          'تم إسناد البوكس بنجاح',
        );
      },
    );

    test(
      'ignores duplicate submit taps while submission is in progress',
      () async {
        boxRepo.nextDetailsResult = const ApiSuccessResult(data: sampleDetails);
        await viewModel.doIntent(const LoadAssignBoxEvent());
    ...
    test(
      'submit 409 conflict emits failure notice and triggers details refresh',
      () async {
        boxRepo.nextDetailsResult = const ApiSuccessResult(data: sampleDetails);
        await viewModel.doIntent(const LoadAssignBoxEvent());
