...
void main() {
  ...
  group('Typed Auth Route Tests', () {
    ...
    testWidgets('VerifyPhoneOtp route with OtpVerificationRouteArgs sets target and role', (
      tester,
    ) async {
      ...
      final otpScreen =
          tester.widget<OtpVerificationScreen>(find.byType(OtpVerificationScreen));
    ...
    testWidgets('AccountStatus route with AccountStatusRouteArgs passes kind', (
      ...
      final statusScreen =
          tester.widget<AccountStatusScreen>(find.byType(AccountStatusScreen));
      expect(statusScreen.kind, AccountStatusKind.moreInformationRequired);
    });

    testWidgets('AppShell route with AppShellRouteArgs routes to driver shell', (
      tester,
    ) async {
      await tester.pumpWidget(
        buildRouteApp(
      ...
      final shell = tester.widget<AppShellScreen>(find.byType(AppShellScreen));
      expect(shell.role, UserRole.driver);
    });

    testWidgets('AppShell route with AppShellRouteArgs routes to dispatcher shell', (
      tester,
    ) async {
      await tester.pumpWidget(
        buildRouteApp(
